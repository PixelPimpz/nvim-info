#!/usr/bin/env b
tmux set-hook -g pane-focus-in[0] 'run "#{@PLUG_ROOT}/get_cur_buffer"' \; 'source-file "$TMUX_ROOT/tmux.conf"'
...
tmux set-hook -g after-refresh-client[11] 'source "$TMUX_ROOT/tmux.conf"'
